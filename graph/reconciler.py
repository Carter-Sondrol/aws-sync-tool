...
class Reconciler:
    ...
    def plan(self, desired: DependencyGraph, target_account: str, *, allow_delete=False) -> Plan:
        changes: List[Change] = []
        # Reverse index: logical_id -> live record (if any) for target account
        live_map = {lid: self._mapping.get(lid, target_account) for lid in self._mapping._by_logical}
        ...
        for node in desired:
            if node.reference_only or node.metadata.get("aws_managed"):
                changes.append(Change(Action.SKIP, node.logical_id, node.service, node.cfn_type, None, None, "reference-only/aws-managed"))
                continue

            live_rec = live_map.get(node.logical_id)
            if not live_rec:
                changes.append(Change(Action.CREATE, node.logical_id, node.service, node.cfn_type, node.properties, None, "not present in target"))
                continue
            ...
            if self._is_equal(desired_spec, live_spec):
                changes.append(Change(Action.NOOP, node.logical_id, node.service, node.cfn_type, None, None, "in sync"))
            else:
                changes.append(Change(Action.UPDATE, node.logical_id, node.service, node.cfn_type, desired_spec, live_spec, "drift detected"))

        # 2) Deletions (live but not desired)
        if allow_delete:
            desired_ids = {n.logical_id for n in desired}
            for lid, rec in live_map.items():
                if lid not in desired_ids and not rec.reference_only and not rec.metadata.get("aws_managed"):
                    changes.append(Change(Action.DELETE, lid, rec.service, rec.cfn_type, None, rec.properties, "not in desired"))
    ...
    def _get_updater(self, service: str):
        from updaters.connect_updater import ConnectUpdater
        from updaters.lambda_updater import LambdaUpdater
        from updaters.iam_updater import IAMUpdater
        match service:
            case "connect": return ConnectUpdater(self._registry)
            case "lambda":  return LambdaUpdater(self._registry)
            case "iam":     return IAMUpdater(self._registry)
            case _:         raise NotImplementedError(f"No updater for {service}")

    def _portable_spec(self, node_or_rec: ResourceNode) -> dict:
        """
        ...
        def walk(o):
            if isinstance(o, dict):
                return {k: walk(map_arnish(v)) for k, v in o.items() if k not in {"LastModified", "RevisionId", "ETag"}}
