		...
		if (instanceId) {
			const listCalls = [
				async () => {
					...
					return r.QueueSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListContactFlowsCommand({ InstanceId: instanceId }),
					);
					return r.ContactFlowSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListContactFlowModulesCommand({ InstanceId: instanceId }),
					);
					return r.ContactFlowModulesSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListRoutingProfilesCommand({ InstanceId: instanceId }),
					);
					return r.RoutingProfileSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListUsersCommand({ InstanceId: instanceId }),
					);
					return r.UserSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListSecurityProfilesCommand({ InstanceId: instanceId }),
					);
					return r.SecurityProfileSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListHoursOfOperationsCommand({ InstanceId: instanceId }),
					);
					return r.HoursOfOperationSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListUserHierarchyGroupsCommand({ InstanceId: instanceId }),
					);
					return r.UserHierarchyGroupSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListQuickConnectsCommand({ InstanceId: instanceId }),
					);
					return r.QuickConnectSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListTaskTemplatesCommand({ InstanceId: instanceId }),
					);
					return r.TaskTemplates || [];
				},
				async () => {
					const r = await client.send(
						new ListRulesCommand({ InstanceId: instanceId }),
					);
					return r.RuleSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListViewsCommand({ InstanceId: instanceId }),
					);
					return r.ViewsSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListPromptsCommand({ InstanceId: instanceId }),
					);
					return r.PromptSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListPhoneNumbersCommand({ InstanceId: instanceId }),
					);
					return r.PhoneNumberSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListAgentStatusesCommand({ InstanceId: instanceId } as any),
					);
					return r.AgentStatusSummaryList || [];
				},
				async () => {
					const r = await client.send(
						new ListEvaluationFormsCommand({ InstanceId: instanceId } as any),
					);
					return r.EvaluationFormSummaryList || [];
				},
			];
			...
			try {
				const structureArn = buildARN(
					"connect",
					`instance/${instanceId}/user-hierarchy-structure/${instanceId}`,
					arn.region,
					arn.accountId,
				);
				...
				console.warn(
					`[connect:instance] failed to construct hierarchy structure ARN:`,
					e,
				);
			}
		}
