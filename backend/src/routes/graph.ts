...
import { parseARN, makeCanonicalName } from '../arn.js';
...
export function createGraphRouter(graphStore: GraphStore) {
  ...
  router.get('/stats', (c) => {
    ...
    for (const node of nodes.values()) {
      ...
      classifications[node.classification] = (classifications[node.classification] || 0) + 1;
    }
  ...
  router.get('/export', (c) => {
    ...
    if (isRaw) {
      ...
      c.header('Content-Disposition', 'attachment; filename="aws-sync-graph.json"');
      c.header('Content-Type', 'application/json');
      return c.body(JSON.stringify(exportData, null, 2));
    }

    const portable = freezeGraph(nodes, edges);
    c.header('Content-Disposition', 'attachment; filename="aws-sync-portable.json"');
    c.header('Content-Type', 'application/json');
    ...
    try {
      ...
      if (validated.type === 'portable') {
        ...
        for (const [arn, nodeData] of Object.entries(nodes)) {
          ...
          const discoveryState = (nodeData.discoveryState || 'resolved') as DiscoveryNode['discoveryState'];
          const classification = (nodeData.classification || 'resource') as DiscoveryNode['classification'];
          const node: DiscoveryNode = {
            logicalId,
            canonicalName: nodeData.canonicalName || makeCanonicalName(parsed.service, logicalId),
  ...
  router.post('/node', async (c) => {
    try {
      ...
      const classification = (body.classification || 'resource') as NodeClassification;
      const node: DiscoveryNode = {
        logicalId: body.logicalId,
        canonicalName: makeCanonicalName(body.service, body.logicalId),
