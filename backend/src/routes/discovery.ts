...
import { getResolver, getAllServices, getResolversByService } from '../resolvers/registry.js';
...
export function createDiscoveryRouter(discoveryEngine: DiscoveryEngine) {
  ...
  router.post('/describe', async (c) => {
    try {
      ...
      const resolverClass = getResolver(`${parsed.service}:${parsed.resourceType}`) || getResolver(parsed.service);
      if (!resolverClass) {
        return c.json({ error: `No resolver for ${parsed.service}:${parsed.resourceType}` });
      }
      ...
      const resolver = new resolverClass(parsed.region || 'us-east-1', credFn);
      const node = await resolver.resolve(parsed);
  ...
  router.post('/expand', async (c) => {
    try {
      ...
      const { seedArns, accountId, maxDepth, excludedServices } = body;
      console.log(`[API] POST /expand seeds=${seedArns.length}, depth=${maxDepth ?? 'default'}, excluded=[${(excludedServices || []).join(', ')}], account=${accountId}`);
      ...
      console.log(`[API] Resolving credentials for profile=${account.profileName}, region=${account.region || 'us-east-1'}`);
      const creds = await resolveCredentials({
        profileName: account.profileName as string,
        region: (account.region as string) || 'us-east-1',
      });
      ...
      discoveryEngine.discover(seedArns, creds, { maxDepth, excludedServices });

      return c.json({ data: { phase: 'running', seedCount: seedArns.length, resolved: 0, totalFound: seedArns.length } });
    } catch (err) {
      return c.json({ error: err instanceof Error ? err.message : String(err) });
    }
  });
  ...
  router.post('/expand-node', async (c) => {
    try {
      ...
      const { arn, accountId, maxDepth, excludedServices } = body;
      console.log(`[API] POST /expand-node arn=${arn}, depth=${maxDepth ?? 'default'}, account=${accountId}`);
      ...
      discoveryEngine.expandFromNode(arn, creds, { maxDepth, excludedServices });

      return c.json({ data: { phase: 'running', seedCount: 1, resolved: 0, totalFound: 1, currentArn: arn } });
    } catch (err) {
      return c.json({ error: err instanceof Error ? err.message : String(err) });
    }
  });
  ...
  router.post('/list', async (c) => {
    try {
      const body = await c.req.json<{ resourceType: string; accountId: string }>();
      const { resourceType, accountId } = body;

      if (!resourceType) {
        return c.json({ error: 'resourceType is required (e.g. "lambda:function")' });
      }
      ...
      const resolverClass = getResolver(resourceType);
      if (!resolverClass) {
  ...
  router.post('/download-lambda-code', async (c) => {
    try {
      ...
      const { functionName, codeBuffer } = await downloadLambdaCode({ arn, credentials: creds });
  ...
  router.post('/resolve', async (c) => {
    try {
      ...
      return c.json({ data: toDiscoveredNode(node.primaryArn.raw ?? arn, node) });
    } catch (err) {
      return c.json({ error: err instanceof Error ? err.message : String(err) });
    }
  });
