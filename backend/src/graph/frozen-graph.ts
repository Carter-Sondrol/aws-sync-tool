...
export function freezeGraph(
  ...
  for (const node of Array.from(nodes.values())) {
    portableNodes[node.logicalId] = {
      ...
      properties: replaceArnsInObj(node.properties, arnMap) as Record<string, unknown>,
  ...
  const services = Array.from(new Set(Array.from(nodes.values()).map((n) => n.service))).sort();
...
export function thawGraph(graph: PortableGraph): {
  ...
  for (const [lid, pn] of Object.entries(graph.nodes)) {
    ...
    const node: DiscoveryNode = {
      logicalId: lid,
      canonicalName: pn.canonicalName,
      service: pn.service,
      cfnType: pn.cfnType,
      ...
      discoveryState: (pn.discoveryState as DiscoveryNode['discoveryState']) ?? 'resolved',
  ...
  const edges: GraphEdge[] = graph.edges.map((e) => {
    ...
    return { id: edgeId, source, target, relationshipType: e.relationshipType, label: e.label };
  });
...
function replaceArnsInObj(obj: unknown, arnMap: Record<string, string>): unknown {
...
function restoreArnsInObj(obj: unknown, arnMap: Record<string, string>): unknown {
  const lidToArn: Record<string, string> = {};
