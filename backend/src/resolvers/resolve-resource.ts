...
export interface ResolveOptions {
  /** Services to skip when extracting referenced ARNs. */
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (ACCESS_DENIED_CODES.has(code)) {
        console.warn(`[${service}] Access denied fetching ${arn.raw} (${code})`);
        return null;
      }
...
function buildNode(
  ...
  return {
    ...
    canonicalName: makeCanonicalName(resolver.service, logicalId ?? arn.resourceId),
    service: resolver.service,
    cfnType: resolver.cfnType ?? `AWS::${capitalize(resolver.service)}::${capitalize(resolver.resourceType)}`,
    properties: data,
    arns,
    referencedArns,
...
function capitalize(str: string): string {
  return str.replace(/([a-z])([A-Z])/g, "$1 $2").replace(/\b\w/g, (c) =>
    c.toUpperCase(),
  );
}
