...
export abstract class BaseResolver {
	...
	protected abstract getClientForRegion(
		service: string,
		region: string,
	): unknown;
	...
	async resolve(arn: ParsedARN): Promise<DiscoveryNode | null> {
		...
		for (let attempt = 0; attempt <= maxRetries; attempt++) {
			try {
				...
				console.log(
					`[Resolver:${service}] fetchResource ← done in ${fetchMs}ms`,
				);
				if (typeof raw !== "object" || raw === null || Array.isArray(raw)) {
					throw new TypeError(
						`fetchResource returned ${typeof raw}, expected object`,
					);
				}
				...
				if (typeof node !== "object" || node === null) {
					throw new TypeError(
						`toNode returned ${typeof node}, expected DiscoveryNode`,
					);
				}

				const totalMs = Date.now() - t0;
				console.log(
					`[Resolver:${service}] ✓ ${arn.raw} → ${node.logicalId} (fetch=${fetchMs}ms, toNode=${nodeMs}ms, total=${totalMs}ms)`,
				);
				return node;
			} catch (err: unknown) {
				const error = err as {
				...
				if (ACCESS_DENIED_CODES.has(code)) {
					...
					this.log.warning(
						`[${service}] Resource not found: ${arn.raw} (${code})`,
					);
					return null;
				} else {
					this.log.error(
		...
		try {
			...
			if (this.config.enableDeepScan) {
				try {
					...
					this.log.warning(
						`[${this.config.service}] deep children failed for ${arn.raw}:`,
						e,
					);
				}
			}
			...
			this.log.warning(
				`[${this.config.service}] extractReferences failed for ${arn.raw}:`,
				e,
			);
	 ...
	 */
	protected async deepChildrenAsync(
		_arn: ParsedARN,
