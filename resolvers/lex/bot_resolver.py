...
class LexBotResolver(BaseLexSubResolver[DescribeBotResponseTypeDef]):
    """Resolves Lex V2 bots and links locales."""
    ...
    def parse(self, arn: ARN, raw: DescribeBotResponseTypeDef) -> ResourceNode[dict[str, Any]]:
        bot = raw.get("bot", raw)
        refs: Set[ARN] = extract_dependencies(bot)

        # Automatically discover locales for this bot
        try:
            ...
            for loc in resp.get("botLocaleSummaries", []):
                ...
                if locale_id:
                    refs.add(ARN(f"arn:aws:lex:{arn.region}:{arn.account_id}:bot/{arn.resource_id}/bot-locale/{locale_id}"))
        except Exception as e:
            logger.warning("[LexBotResolver] list_bot_locales failed for %s: %s", arn, e)
        ...
        return ResourceNode(
            ...
            arns={"Bot": arn},
            metadata={"BotStatus": bot.get("botStatus")},
        )
